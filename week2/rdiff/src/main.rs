...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    let len_seq1 = seq1.len();
    let len_seq2 = seq2.len();
    let mut seq_grid = Grid::new(len_seq1+1, len_seq2+1);

    for i in 0..len_seq1 {
        for j in 0..len_seq2 {
            if seq1[i] == seq2[j] {
                seq_grid.set(i+1, j+1, seq_grid.get(i, j).unwrap() + 1).unwrap();
            } else {
                let t1 = seq_grid.get(i+1, j).unwrap();
                let t2 = seq_grid.get(i, j+1).unwrap();
                seq_grid.set(i+1, j+1,if t1 > t2 {t1} else {t2}).unwrap();
            }
        }
    }
    seq_grid
